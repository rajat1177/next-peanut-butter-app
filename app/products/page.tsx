...
import { useUser, useClerk } from "@clerk/nextjs"; // 1. Import Clerk hooks
...
export default function ProductsShowcase() {
  ...
  // Optional: If you want the login to pop up IMMEDIATELY when page loads (if not logged in)
  ...
  */
  ...
  async function addToCart(variantId: string, productId: string) {
    // 3. Clerk Check: Pop up login if user is "not there"
    if (!isSignedIn) {
      openSignIn();
      return;
    }
      ...
      <div className="pt-24 max-w-7xl mx-auto px-6 py-14">
        ...
        {isLoading ? (
          <div className="flex flex-col justify-center items-center h-64">
            ...
            <p className="text-white/60 animate-pulse">Loading Collection...</p>
          </div>
        ) : (
          <>
            {products.length === 0 && (
                  ...
                  >
                    ...
                    <div className="px-5 pb-5 space-y-3">
                      {defaultVariantId && (
                        <div className="flex items-center justify-between bg-white/5 border border-white/20 rounded p-1">
                          ...
                          <div className="flex items-center gap-3">
                            ...
                            <span className="text-white font-mono w-4 text-center">
                              {currentQty}
                            </span>
                            <button
                              onClick={() => handleQuantityChange(product.id, 1)}
                      ...
                      {defaultVariantId && (
                        ...
                        >
                          {/* Optional: Change text if not signed in */}
                          {adding === defaultVariantId 
                            ? "Adding..." 
                            : !isSignedIn 
                              ? "Sign in to Buy" 
                              : "Add to Cart"}
